...
class JuegoAventuraGUI:
    """Interfaz gráfica del juego usando Tkinter"""
    # ...resto del código...
    ...
    def __init__(self, root):
        ...
        # Variables
        self.imagenes_cache = {}
        # Inicializar audio (intenta reproducir musica de fondo si pygame está disponible)
    ...
    # ------------------- Audio (opcional, usa pygame si está disponible) -------------------
    def inicializar_audio(self):
        """Intentar inicializar el sistema de audio y cargar la pista de fondo.
        Si pygame no está disponible o el archivo no existe, desactiva el audio silenciosamente.
        """
        ...
        if not PYGAME_AVAILABLE:
            # pygame no está instalado; no hacemos nada
            return
    ...
    def detener_audio(self):
        """Detener y limpiar el sistema de audio si está activo."""
    ...
    def crear_placeholder(self, ancho, alto, texto):
        ...
        # Aquí podrías agregar texto a la imagen usando ImageDraw
        return ImageTk.PhotoImage(img)
...
###segundo bloque de codigo final
    ...
    def personaje_seleccionado(self, personaje):
        """Maneja la selección de un personaje y muestra la dificultad."""
    ...
    def mostrar_seleccion_dificultad(self, nombre_personaje, titulo_personaje):
        ...
        # Textos de dificultad dinámicos según el personaje
        ...
        for i, dif in enumerate(dificultades):
            btn = tk.Button(
                ...
                height=5,
                cursor="hand2",
                command=lambda d=dif["dificultad_id"]: self.iniciar_juego(nombre_entry.get(), d)
            )
    ...
    def iniciar_juego(self, nombre, dificultad):
        ...
        # Determinar nodo inicial adaptándose a los prefijos reales de los nodos
    ...
    def cargar_partida(self):
        ...
        if self.juego.cargar_partida():
            messagebox.showinfo("Éxito", "Partida cargada correctamente")
            self.mostrar_nodo(self.juego.jugador.nodo_actual)
        else:
            messagebox.showerror("Error", "No se encontró ninguna partida guardada")
...
• Persistencia de datos (guardado/cargado)
• Tres niveles de dificultad
• Múltiples finales
• Narrativa interactiva ramificada

TECNOLOGÍA:
• Lenguaje: Python 3.x
• GUI: Tkinter
• Bibliotecas: PIL/Pillow, json

PALETA DE COLORES:
• Rojo brillante (#DF2531)
• Rojo medio (#B22222)
• Rojo oscuro (#8B1A1A)
• Blanco (#FFFFFF)
• Negro (#000000)
...
¡Gracias por jugar!
    ...
    def crear_panel_stats(self, parent):
        """Crear panel de estadísticas del jugador"""
        ...
        tk.Label(
            stats_frame,
            text=f"👤 {self.juego.jugador.nombre}",
            font=("Arial", 14, "bold"),
            fg=self.COLOR_TEXTO,
            bg=self.COLOR_ROJO_OSCURO
        ...
        tk.Label(
            stats_container,
            text=f"❤️ Salud: {self.juego.jugador.salud}/100",
            font=("Arial", 12),
            fg=self.COLOR_TEXTO,
            bg=self.COLOR_ROJO_OSCURO
        ...
        tk.Label(
            stats_container,
            text=f"⭐ Reputación: {self.juego.jugador.reputacion}/100",
            font=("Arial", 12),
            fg=self.COLOR_TEXTO,
            bg=self.COLOR_ROJO_OSCURO
        ...
        tk.Label(
            stats_container,
            text=f"💎 Recursos: {self.juego.jugador.recursos}",
            font=("Arial", 12),
            fg=self.COLOR_TEXTO,
            bg=self.COLOR_ROJO_OSCURO
        ...
        # Inventario
        inv_text = f"🎒 Inventario: {len(self.juego.jugador.inventario)} items"
        tk.Label(
            stats_container,
            text=inv_text,
            font=("Arial", 12),
    ...
    def crear_panel_opciones(self, parent, nodo):
        """Crear panel de opciones de decisión"""
    ...
    def elegir_opcion(self, nodo, opcion):
        ...
        # Con la refactorización sugerida, esto sería más limpio:
        cambios = []
...
Reputación final: {self.juego.jugador.reputacion}/100
...
Items obtenidos: {len(self.juego.jugador.inventario)}
    ...
    def mostrar_inventario(self):
        ...
        ventana.title("Estadísticas e Inventario")
        ventana.geometry("500x400")
        ventana.configure(bg=self.COLOR_FONDO)

        tk.Label(
            ventana,
            text="📊 ESTADÍSTICAS E INVENTARIO",
            font=("Arial", 16, "bold"),
            fg=self.COLOR_ROJO_BRILLANTE,
            bg=self.COLOR_FONDO
        ).pack(pady=10)
...
ESTADÍSTICAS:
❤️  Salud: {self.juego.jugador.salud}/100
⭐ Reputación: {self.juego.jugador.reputacion}/100
💎 Recursos: {self.juego.jugador.recursos}
...
"""
    ...
    def volver_menu_confirmacion(self):
        """Confirmar antes de volver al menú principal"""
        respuesta = messagebox.askyesno(
            "Confirmar",
            "¿Seguro que quieres volver al menú principal?\n"
            "Asegúrate de haber guardado tu progreso."
        )
        if respuesta:
            self.crear_menu_principal()
